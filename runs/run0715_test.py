...
for dataset in ["prompts"]:
    for leace in [None]:
        for start in starts:
            cmd = f"python {ROOT}steer.py --dataset {dataset} --mults {start} {start+0.1} --layer all"
            if leace is not None:
                cmd += f" --leace {leace}"
            commands[f"steer_{dataset}_{leace}_{start}"] = cmd

            cmd = f"python {ROOT}eval.py --dataset {dataset} --mults {start} {start+0.1} --layer all --port {port} --server"
            if leace is not None:
                cmd += f" --leace {leace}"
