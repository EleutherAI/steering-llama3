...
for dataset in ["abcon", "ab", "openr"]:
    for sign in ['-', '+']:
        cmd = f"python steer.py --dataset {dataset} --mults {sign}2.5 {sign}3 {sign}3.5 {sign}4"
        # if leace is not None:
        #     cmd += f" --leace {leace}"
        commands[f"steer_{dataset}_{sign}"] = cmd

        cmd = f"python eval.py --dataset {dataset} --mults {sign}2.5 {sign}3 {sign}3.5 {sign}4 --port {port} --server"
        # if leace is not None:
        #     cmd += f" --leace {leace}"
        commands[f"eval_{dataset}_{sign}"] = cmd
        port += 1
