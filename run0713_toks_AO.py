...
for dataset in ["ab", "prompts"]:
    for toks in ["before", "after"]:
        ...
        cmd = f"python eval.py --dataset {dataset} --mults $(seq -2 .2 2) --toks {toks} --port {port} --server"
        # if leace is not None:
        #     cmd += f" --leace {leace}"
        commands[f"eval_{dataset}_{toks}"] = cmd
        port += 1

        cmd = f"python steer.py --dataset {dataset} --mults $(seq -1 .1 1) --toks {toks} --residual --layer all"
        # if leace is not None:
        #     cmd += f" --leace {leace}"
        commands[f"steer_res_{dataset}_{toks}"] = cmd

        cmd = f"python eval.py --dataset {dataset} --mults $(seq -1 .1 1) --toks {toks} --residual --layer all --port {port} --server"
        # if leace is not None:
        #     cmd += f" --leace {leace}"
