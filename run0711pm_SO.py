...
for dataset in ["prompts"]:
    for leace in [None, "leace", "orth"]:
        cmd = f"python steer.py --dataset {dataset} --mults $(seq 0 .125 3) --layer all --residual"
        if leace is not None:
            cmd += f" --leace {leace}"
        commands[f"steer_res_{dataset}_{leace}"] = cmd

        cmd = f"python eval.py --dataset {dataset} --mults $(seq 0 .125 3) --layer all --residual --port {port} --server"
        if leace is not None:
            cmd += f" --leace {leace}"
